...
def main() -> None:
    try:
        ...
        raise RuntimeError("psycopg2-binary is required. Run: uv sync") from exc

    db_url = os.getenv(SUPABASE_DB_URL_ENV, "").strip()
    if not db_url:
        raise RuntimeError(f"Missing {SUPABASE_DB_URL_ENV} environment variable")
